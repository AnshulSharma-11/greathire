...
function buildRecord(employee, date) {
  ...
  const status = isWeekend ? "Weekend" : pick(STATUSES);
  ...
  if (status === "Present" || status === "Late") {
    late = status === "Late";
    const hour = late ? 9 : 8;
    const minute = late ? 5 + Math.floor(rand() * 30) : 45 + Math.floor(rand() * 15) - 15;
...
export const attendanceRecords = seedAttendance(60);
