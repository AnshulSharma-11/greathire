...
function roleForEmployee(employee) {
  ...
  if (employee.role?.toLowerCase().includes("manager") || employee.role?.toLowerCase().includes("lead")) return "manager";
  return "employee";
}
...
export async function seedDatabaseIfEmpty() {
  ...
  if ((await UserModel.countDocuments()) === 0) {
    ...
    console.log(`[seed] inserted ${users.length} users (demo password: ${demoPassword})`);
  }
  ...
  if ((await NotificationPreferenceModel.countDocuments()) === 0) {
    ...
    console.log("[seed] inserted notification preferences");
  }
  ...
  if ((await ChannelModel.countDocuments()) === 0 && (await DirectConversationModel.countDocuments()) === 0) {
    ...
    console.log(`[seed] inserted ${channels.length} channels, ${directConversations.length} DMs, ${messages.length} messages`);
  }

  if ((await AnnouncementModel.countDocuments()) === 0) {
    await AnnouncementModel.insertMany(buildAnnouncements());
    console.log("[seed] inserted announcements");
  }

  if ((await HolidayModel.countDocuments()) === 0) {
    await HolidayModel.insertMany(buildHolidays());
    console.log("[seed] inserted holidays");
  }
}
