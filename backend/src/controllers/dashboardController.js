...
export let dashboardController = {
  ...
  getLiveWorkforce: (req, res) => {
    let { limit } = req.query;
    res.json({ success: true, data: Dashboard.getLiveWorkforce(limit ? Number(limit) : 10) });
  },
  ...
  getRecentActivity: (req, res) => {
    let { limit } = req.query;
    res.json({ success: true, data: Dashboard.getRecentActivity(limit ? Number(limit) : 10) });
  },
