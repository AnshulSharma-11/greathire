...
function periodRows(range, department) {
  ...
  const rows = Attendance.getRange(cutoff, todayISO());
...
function previousPeriodRows(range, department) {
  ...
  const rows = Attendance.getRange(start, end);
  if (!department || department === "All Departments") return rows;
  return rows.filter((r) => r.employee?.department === department);
}
...
export const Report = {
  ...
  getStatsCards(range = "12m", department) {
    const current = periodRows(range, department);
    const previous = previousPeriodRows(range, department);

    const totalEmployees = Employee.getAll(department).length;
    const prevHeadcount = new Set(previous.map((r) => r.employeeId)).size || totalEmployees;
  ...
  getAttendanceTrends(range = "12m", department) {
    const rows = periodRows(range, department);
  ...
  getWorkingHoursTrend(range = "12m", department) {
    const rows = periodRows(range, department).filter((r) => r.hoursWorked > 0);
    const byDate = new Map();
