  ...
  return (
    <header className="flex items-center justify-between gap-4 px-6 py-4 bg-white border-b border-slate-200 flex-shrink-0">
      ...
      <div className="flex items-center gap-5 flex-shrink-0">
        ...
        <button className="text-sm font-medium text-blue-600 hover:text-blue-700">Support</button>
  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="text-sm text-slate-500 mt-1">Stay updated with workforce activities and important alerts.</p>
      </div>
          ...
          {filters.map((filter) => (
            <button
              ...
              className={`relative px-3.5 py-1.5 rounded-full text-sm font-medium transition-colors ${
                activeFilter === filter ? "bg-slate-900 text-white" : "text-slate-500 hover:bg-slate-100"
              }`}
            >
              {filter}
            </button>
...
function NotificationRow({ item, onClick }) {
  ...
  return (
    <div
      onClick={onClick}
      className={`flex items-start gap-4 px-6 py-5 border-b border-slate-100 last:border-b-0 cursor-pointer ${
...
function SummaryCard({ summary }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5">
      <h3 className="text-base font-bold text-slate-900 mb-4 pb-3 border-b border-slate-100">Summary</h3>
...
function QuickActionsCard({ onOpenPreferences }) {
  return (
    <div className="bg-white rounded-2xl border border-slate-200 p-5">
      <h3 className="text-base font-bold text-slate-900 mb-4">Quick Actions</h3>
      <div className="flex flex-col gap-2">
        <button onClick={onOpenPreferences} className="flex items-center gap-2 border border-slate-200 rounded-lg px-3 py-3 text-sm font-medium text-slate-700 hover:bg-slate-50">
            ...
            <div className="xl:col-span-2 min-w-0">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
                {notifications.length === 0 ? (
                  <p className="px-6 py-10 text-center text-sm text-slate-400">No notifications here.</p>
                ) : (
